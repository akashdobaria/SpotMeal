...
//    private HashMap<String,ArrayList<String >> drinks = new HashMap<String,ArrayList<String >>();
    ...
    public void validateFields(){
        ...
        if( this.starters.size()== 0)
        {
            validInput = false;
            ((EditText)findViewById(R.id.starters_input1)).setError( "All details is required!" );
            ((EditText)findViewById(R.id.starters_input1 )).setHint("please enter details");

        }
        if( this.mainCourse.size()== 0)
        {
            validInput = false;
            ((EditText)findViewById(R.id.main_course_input1)).setError( "All details is required!" );
            ((EditText)findViewById(R.id.main_course_input1)).setHint("please enter details");
        }
        if( this.deserts.size()== 0)
        {
            validInput = false;
            ((EditText)findViewById(R.id.desert_input1)).setError( "All details is required!" );
            ((EditText)findViewById(R.id.desert_input2)).setHint("please enter details");
        }
        if( this.drinks.size()== 0)
        {
            validInput = false;
            ((EditText)findViewById(R.id.drinks_input1)).setError( "All details is required!" );
            ((EditText)findViewById(R.id.drinks_input2)).setHint("please enter details");
        }
        if( this.costPerPerson.trim() == "")
        {
            validInput = false;
            ((EditText)findViewById(R.id.cost)).setError( "All details is required!" );
            ((EditText)findViewById(R.id.cost)).setHint("please enter details");
        }
        if( this.eventDate.trim().equals(""))
        {
            validInput = false;
            ((EditText)findViewById(R.id.date_input)).setError( "All details is required!" );
            ((EditText)findViewById(R.id.date_input)).setHint("Please select event date");
        }
        if( this.latitude.trim().equals(""))
        {
            validInput = false;
            ((TextView)findViewById(R.id.selected_location)).setText( "Please select location" );
            ((TextView)findViewById(R.id.selected_location)).setTextColor(getResources().getColor(R.color.colorAccent));

        }
        if(  this.longitude.trim().equals(""))
        {
            validInput = false;
            ((TextView)findViewById(R.id.selected_location)).setText( "Please select location" );
            ((TextView)findViewById(R.id.selected_location)).setTextColor(getResources().getColor(R.color.colorAccent));
...
//                    Log.w(TAG, "postComments:onCancelled", databaseError.toException());
            ...
            if(validInput)
            {
                ...
                this.eventDate = dateFormat.format(new Date(this.eventDate));

                Event newEvent = new Event(this.uid, this.title, this.cuisine, this.spiceLevel,this.starters, this.mainCourse, this.deserts, this.drinks, this.minGuests, this.maxGuests, Long.valueOf(this.costPerPerson), this.eventDate, this.startTime, this.endTime,this.latitude, this.longitude, 0L, HomeActivity.mCurrentUser);
...
//                mDatabase.child("users_p").child(auth.getGivenBy()).child("events_hosting").push().setValue(newEvent);
...
//                        Toast.makeText(UploadImageTry.this, "Select an image", Toast.LENGTH_SHORT).show();
        ...
        public void showPicturesPreview(ArrayList<String> selectedPictures){
